...
public class Fabrica {
	...
	Album crearAlbumWeb() {
        AlbumWeb album = new AlbumWeb(paisesParticipantes);
        return album;
	}

	Album crearAlbumExtendido() {
		AlbumExtendido album = new AlbumExtendido(paisesParticipantes);
        return album;
	}

	Album crearAlbumTradicional() {
		AlbumTradicional album = new AlbumTradicional(paisesParticipantes,random.nextInt(10)+1);
        return album;
	}
	...
	List<Figurita> generarSobre(int cantFigus) {
		...
		for (int i = 0; i < cantFigus; i++) {
			String pais = paisesParticipantes[random.nextInt(32)];
			Integer numJugador = (random.nextInt(12) + 1);
			Integer valorBase = calcularValorBase(pais,numJugador);
			double valorFinal = ranking.get(pais) * valorBase;
			Figurita fig = new Figurita(numJugador,pais,valorBase, valorFinal);
			sobre.add(fig);
		}
		return sobre;
	}		
	...
	List<Figurita> generarSobreTop10(int cantFigus) {
		ArrayList<Figurita> sobre = new ArrayList<Figurita>();
		for (int i = 0; i < cantFigus; i++) {
			String paisSede = listadoDeMundialesTop10[random.nextInt(10)];
			Integer valorBase = (random.nextInt(10)+1);;
			String [] finalista = balonYPaisPorMundialTop10.get(paisSede);
			int puesto = random.nextInt(2) +1;
			String tipoBalon;
			double valorFinal;
			if(puesto == 1) {
				tipoBalon = "Oro";
				valorFinal = (ranking.get(finalista[puesto]) * valorBase)*1.2;
			} else {
				tipoBalon = "Plata";
				valorFinal = (ranking.get(finalista[puesto]) * valorBase)*1.1;
			}
			FiguritaTop10 fig = new FiguritaTop10(paisSede , finalista[puesto],tipoBalon, valorBase,valorFinal);
			sobre.add(fig);
		}
		return sobre;
	}
